...
/*   Updated: 2018/08/04 15:50:17 by adzikovs         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned long	get_big_endian_int(void *ptr, size_t size)
{
	unsigned long	res;
	size_t			i;
...
uint32_t		get_nfat_arch(void *ptr)
{
...
void			*get_file_ptr(void *ptr, size_t i)
{
	struct fat_header		*header;
	struct fat_arch			*f_hdr;
	uint32_t				offset;
	size_t					size;
	...
	{
		offset = (uint32_t)get_big_endian_int(&(f_hdr->offset), size);
		return (ptr + offset);
	}
	else
		return (NULL);
...
int				handle_fat(void *ptr)
{
	...
	nfat_arch = get_nfat_arch(ptr);
	i = 0;
	while (i < nfat_arch)
	{
		file_ptr = get_file_ptr(ptr, i);
